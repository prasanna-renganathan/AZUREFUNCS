...
namespace Pluralsight.AzureFuncs
{
    public class OrderOrchestrationInput
    {
        ...
        //Adding total price to implement manual approval, when the price is above a certain limit.
        public decimal TotalPrice { get; set; }
    }

    //Payload of the approval event.
    ...
    public static class NewOrderOrchestration
    {
            ...
            [OrchestrationTrigger] TaskOrchestrationContext context)
        {
            ...
            if (orderInput.TotalPrice >= 500)
            {
                ...
                // calls request approval to approve the ticket
                await context.CallActivityAsync(nameof(RequestApproval), orderInput.OrderId);
                try
                {
                    /*
                    Requested approval, WaitForExternalEvent method on the task orchestration context is used.
                    Where the event name is passed that we're waiting for.
                    A strongly typed object can be supplied for it to deserialize the event's payload into. 
                    In this case, it's an ApprovalResult.
                    */
            ...
            //if the ticket is approved, fan in fan out pattern is used to create the tickets
        ...
        //Approve method handles the actual approval event to the orchestrater.
        //In this case we are using DurableTaskClient.
